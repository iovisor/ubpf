...
int main(int argc, char **argv)
{
    ...
    ubpf_set_inst_cnt_enable(vm, inst_cnt);
    ...
    printf("0x%"PRIx64"\n", ret);
    if (inst_cnt) {
	struct ubpf_inst_cnt ubpf_inst_cnt;
	ubpf_inst_cnt = ubpf_get_inst_cnt(vm);
        printf("\nInstruction counts (cmpl/vm/bm): %zd / %zd / %zd\n",
            ubpf_inst_cnt.inst_cmpl, ubpf_inst_cnt.inst_vm, ubpf_inst_cnt.inst);
    }
    ubpf_destroy(vm);
